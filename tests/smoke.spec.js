...
test('main UI boots and renders v3 modules', async ({ page }) => {
  ...
  const realmHeaderState = await page.locator('#panel-cult .section').first().evaluate(section => {
    const label = section.querySelector('#realm-type-label');
    return {
      labelHidden: label && getComputedStyle(label).display === 'none',
      labelText: label?.textContent || '',
      buttonText: Array.from(section.querySelectorAll('.section-header button')).map(btn => btn.textContent.trim()).join('|'),
    };
  });
  expect(realmHeaderState.labelHidden).toBe(true);
  expect(realmHeaderState.labelText).toBe('');
  expect(realmHeaderState.buttonText).toContain('修为');
  expect(realmHeaderState.buttonText).toContain('炼体');
  const realmMetricLayout = await page.locator('#realm-metrics').evaluate(metrics => {
    const previous = metrics.previousElementSibling.getBoundingClientRect();
    ...
    return {
      ...
      gapFromLevelBar: Math.round(metricsBox.top - previous.bottom),
      cards,
    };
  });
  ...
  expect(realmMetricLayout.gapFromLevelBar).toBeLessThanOrEqual(6);
  expect(realmMetricLayout.cards[0].text).toMatch(/基础战力：\d/);
  const realmChartLabels = await page.locator('#panel-cult .chart-row svg text').evaluateAll(nodes => nodes.map(node => node.getAttribute('fill')));
  expect(realmChartLabels).toEqual(['var(--text)', 'var(--text)', 'var(--text)']);
  ...
  await page.locator('#eco-output-section .section-collapse-toggle').click();
  await expect(page.locator('#eco-output-section')).not.toHaveClass(/is-collapsed/);
  await expect(page.locator('#tbl-eco-config')).toContainText('编辑');
  await expect(page.locator('#tbl-eco-config')).toContainText('删除');
  await page.locator('#tbl-eco-config button[onclick="editEcoOutputRow(\\\'r1\\\')"]').click();
  await page.locator('#tbl-eco-config input[data-rid="r1"][onchange*="dayProd"]').fill('3333');
  await page.locator('#tbl-eco-config button[onclick="saveEcoOutputRow(\\\'r1\\\')"]').click();
  await expect(page.locator('#tbl-eco-config')).toContainText('3333');
  ...
  const classPanelLayout = await page.locator('#class-selector').evaluate(panel => {
    ...
    const actions = panel.querySelector('.class-card .class-actions').getBoundingClientRect();
    const card = panel.querySelector('.class-card').getBoundingClientRect();
    ...
    const firstRowTops = statItems.slice(0, 5).map(item => Math.round(item.top));
    const secondRowTop = Math.round(statItems[5]?.top || 0);
    return {
      ...
      firstFiveShareRow: firstRowTops.length === 5 && firstRowTops.every(top => top === firstRowTops[0]),
      sixthStartsNextRow: statItems.length < 6 || secondRowTop > firstRowTops[0],
      statPairGap: Math.round(firstValue.left - firstLabel.right),
      labelLeftOfValue: firstLabel.right <= firstValue.left,
      actionsBelowStats: actions.top > stats.getBoundingClientRect().bottom,
      actionsHasSeparator: getComputedStyle(panel.querySelector('.class-card .class-actions')).borderTopWidth !== '0px',
      actionsInsideCard: actions.bottom <= card.bottom,
    };
  });
  expect(classPanelLayout.sameCenterLine).toBe(true);
  expect(classPanelLayout.inputHeight).toBe(32);
  expect(classPanelLayout.gridColumns).toBe(5);
  expect(classPanelLayout.firstFiveShareRow).toBe(true);
  expect(classPanelLayout.sixthStartsNextRow).toBe(true);
  expect(classPanelLayout.labelLeftOfValue).toBe(true);
  expect(classPanelLayout.statPairGap).toBeLessThanOrEqual(6);
  expect(classPanelLayout.actionsBelowStats).toBe(true);
  expect(classPanelLayout.actionsHasSeparator).toBe(true);
  expect(classPanelLayout.actionsInsideCard).toBe(true);
  ...
  const slotLayout = await page.locator('#slot-editor').evaluate(editor => {
    const card = editor.querySelector('.slot-card');
    const header = card.querySelector('.slot-header');
    const name = header.querySelector('.slot-name').getBoundingClientRect();
    const type = header.querySelector('.slot-type').getBoundingClientRect();
    const attrs = card.querySelector('.slot-attrs');
    const attrItems = Array.from(attrs.querySelectorAll('span')).map(item => item.getBoundingClientRect());
    return {
      cardWidth: Math.round(card.getBoundingClientRect().width),
      headerSameRow: Math.abs((name.top + name.height / 2) - (type.top + type.height / 2)) < 3,
      typeAfterName: type.left > name.right,
      attrColumns: getComputedStyle(attrs).gridTemplateColumns.split(' ').length,
      firstThreeShareRow: attrItems.length < 3 || attrItems.slice(0, 3).every(item => Math.round(item.top) === Math.round(attrItems[0].top)),
      fourthNextRow: attrItems.length < 4 || Math.round(attrItems[3].top) > Math.round(attrItems[0].top),
    };
  });
  expect(slotLayout.cardWidth).toBeGreaterThanOrEqual(250);
  expect(slotLayout.headerSameRow).toBe(true);
  expect(slotLayout.typeAfterName).toBe(true);
  expect(slotLayout.attrColumns).toBe(3);
  expect(slotLayout.firstThreeShareRow).toBe(true);
  expect(slotLayout.fourthNextRow).toBe(true);
